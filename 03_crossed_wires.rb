...
def add_line(field, field_steps, c_start, total_steps, direction, steps, field_value)
	x = c_start[0]
	y = c_start[1]
	dx = 0
	dy = 0
	...
	steps.times { |t|
		...
		field_steps[[x, y, field_value]] = total_steps + t
		if field[[x, y]] == 3
			$min_manhattan = [$min_manhattan, x.abs + y.abs ].min
			$min_intersect = [$min_intersect, field_steps[[x, y, 1]] + field_steps[[x, y, 2]] ].min
			printf("W1: #{field_steps[[x, y, 1]]}, W2: #{field_steps[[x, y, 2]]}\n")
		end
	}
	return [x, y], total_steps+steps
end

$min_manhattan = 100000000000
$min_intersect = 100000000000
trace_var :$min_intersect, proc{print "$min_intersect is now ", $min_intersect, "\n"}

def add_wire(field, field_steps, wire, field_value)
	act_coords = [0, 0]

	total_steps = 1
	wire.each {|direction, steps|
		act_coords, total_steps = add_line(field, field_steps, act_coords, total_steps, direction, steps, field_value)
	}
end

field = Hash.new(0)
field_steps = Hash.new(0)
add_wire(field, field_steps, wire1, 1)
add_wire(field, field_steps, wire2, 2)

printf("P1 min manhattan of intersection: #{$min_manhattan}\n")
printf("P2 min intersection steps: #{$min_intersect}\n")
